...
public class SphenicNumber
{
     ...
     * to first store if the current's thread maximum value multiplied by itself
     * three times is less than the value we are searching.
     * If this is true (the min^3 > sphenic value), and at any point the algorithm obtains
     * a bigger number than the value we are testing, we know the value we are testing
     * cannot be a sphenic number, thus the "notPossibleSphenic" variable is set to true.
     *
    ...
    {
        long a, b, c;
        boolean minFound = primeList.get((int) max) * primeList.get((int) max) * primeList.get((int) max) < value;
        ...
        for(long i = min; i <= max; i++)
        {
            ...
            for(long j = i + 1; j < primeList.size() - 1; j++)
            {
                ...
                if(a * b * b > value) break; //Break because number is too large.
                for(long k = j + 1; k < primeList.size(); k++)
                {
                    c = primeList.get((int) k);

                    if(terminate) return true;
                    else if((a * b *  c) == value) return true; //sphenic found
                }
            }
        }
        return false;
